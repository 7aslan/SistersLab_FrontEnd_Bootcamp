const pokemons = [
  ...
  const waterPokemonsCount = waterPokemons.length;
  const waterPokemonsName = waterPokemons.map(pokemon => pokemon.name);


  console.log(`"Water" türündeki Pokemon sayısı:  ` + waterPokemonsCount);
  console.log(`"Water" türündeki Pokemon isimleri: ` + waterPokemonsName);
  ...
  //Pokemon dizisinde, her türdeki Pokemon'ların deneyim puanlarının ortalamasını hesaplamak istiyorsunuz.
    ...
    pokemons.forEach(pokemon => {
        ...
        // Create a new list if the type is not defined
        if(!typeExperienceAverages[type]) {
            typeExperienceAverages[type] = { totalExperience: 0, count: 0 };
        }

        // Sum experience score and increase the count 
        typeExperienceAverages[type].totalExperience += experience;
        typeExperienceAverages[type].count++;

    })
    
    // Calculate the average value for each type of pokemon
    for (const type in typeExperienceAverages){
        const typeData = typeExperienceAverages[type];
        typeData.averageExperience = typeData.totalExperience / typeData.count;
    }
